...
/* fill the next hop mac address and send the packet */
void sr_fillMacSend(struct sr_instance* sr, 
        uint8_t *packet,
        unsigned int len,
        char* interface,
		struct in_addr hop_ip,
		struct sr_if *send_if)
{
    ...
    struct arp_cache *macAddr = Look_up_ARPCache(sr, hop_ip);

	struct sr_ethernet_hdr *forward_ethernet = (struct sr_ethernet_hdr *)packet;
	memcpy(forward_ethernet->ether_shost, send_if->addr, ETHER_ADDR_LEN);

    if (macAddr != NULL)
    {
        memcpy(forward_ethernet->ether_dhost, macAddr->address, ETHER_ADDR_LEN);
	...
	else 
	{
		...
		Add_Message_Entry(sr, waiting_packet, len, interface_temp, interface, hop_ip);
		free(waiting_packet);
	}
}
...
void sr_handleIPpacket(struct sr_instance* sr, 
        ...
        char* interface)
{
	//struct in_addr hello_broadcast_addr[4] = {0x01, 0x00, 0x5e, 0x00, 0x00, 0x05};
    struct ip *ip_hdr = NULL;
    ip_hdr = (struct ip *)(sizeof(struct sr_ethernet_hdr) + packet);
    ...
    else 
    {
        ...
        {
            if (temp_interface->ip == ip_hdr->ip_dst.s_addr || ip_hdr->ip_dst.s_addr == 83886304)
            {
                ...
                else if(ip_hdr->ip_p == IPPROTO_PWOSPF)
                {
                	sr_handlePWOSPF(sr, packet, len, interface);
                }
                else 
                {
